...
prop_lte (S j) 0 x = absurd x
prop_lte (S j) (S i) (LTESucc x) = LTESucc (prop_lte j i x)

prop_lte_n_n : (n : Nat) -> LTE n n
prop_lte_n_n 0 = LTEZero
prop_lte_n_n (S k) = LTESucc (prop_lte_n_n k)

prop_lte_2xp1 : (n, m : Nat) -> LTE n m -> LTE (plus n (S n)) (plus m (S (S m)))
prop_lte_2xp1 0 0 x = LTESucc LTEZero
prop_lte_2xp1 0 (S k) x = LTESucc LTEZero
prop_lte_2xp1 (S k) (S j) (LTESucc x) = 
    let t = prop_lte_2xp1 k j x in
    LTESucc (
        rewrite sym $ plusSuccRightSucc k (S k) in
        rewrite sym $ plusSuccRightSucc j (S (S j)) in
        LTESucc t
    )
...
prop_filterX_len : 
    ...
    {0 rel : ty -> ty -> Type} ->
    (decide : ((z, y : ty) -> Dec (rel z y))) -> 
    (mmm : Nat) -> (vvv : Vect mmm ty) -> {w : ty} -> (len : Nat) -> (l : Vect len ty) -> 
    (prfDP : (mmm ** vvv) = filterX' decide w l) -> LTE mmm len
--                dec  mmm vvv len l prfDP
prop_filterX_len decide 0 [] 0 [] prfDP = LTEZero
prop_filterX_len decide (S mmm) (x :: xs) 0 [] prfDP = absurd (cong fst prfDP)
prop_filterX_len decide 0 [] (S len) (x :: xs) prfDP = LTEZero
prop_filterX_len decide (S mmm) (y :: ys) (S len) (x :: xs) prfDP with (decide w x) proof eq_dwx
  prop_filterX_len decide (S mmm) (y :: ys) (S len) (x :: xs) prfDP | (Yes eq_wx) =
    let ih = prop_filterX_len decide (S mmm) (y :: ys) len xs {w=w} prfDP in
    LTESucc (prop_lte mmm len ih)
  prop_filterX_len decide (S mmm) (y :: ys) (S len) (x :: xs) prfDP | (No ne_wx) with (filterX' decide w xs) proof eq_fwxs
    prop_filterX_len decide (S m2) (x :: v2) (S len) (x :: xs) Refl | (No ne_wx) | (m2 ** v2) = 
      let ih = prop_filterX_len decide m2 v2 len xs {w=w} (sym eq_fwxs) in
      LTESucc ih
...
combiX_sound1 :
    ...
    SubVect v l
--            n l  v   l'  prf  prfElem
combiX_sound1 0        _ _ _ {cond} _ _ _ _ =
    let cond' = replace {p = \c => LTE (c) 0} (sym $ plusSuccRightSucc _ len) cond in
    absurd cond'
combiX_sound1 (S fuel) decEqX 0 [] [] [[]] Refl prfElem = SVNil
combiX_sound1 (S fuel) decEqX 0 (x :: xs) [] [[]] Refl prfElem = SVNil
combiX_sound1 (S fuel) decEqX (S _) [] (_ :: _) [] Refl Here impossible -- l = [], l' = []
combiX_sound1 (S fuel) decEqX (S _) [] (_ :: _) [] Refl (There later) impossible -- l = [], l' = []
combiX_sound1 (S fuel) decEqX (S k) [] (y :: ys) (z :: zs) prf prfElem = 
    let absurdPrf = cong fst prf in
    absurd absurdPrf
combiX_sound1 (S fuel) decEqX (S _) (_ :: _) (_ :: _) [] _ Here impossible
combiX_sound1 (S fuel) decEqX (S _) (_ :: _) (_ :: _) [] _ (There later) impossible
combiX_sound1 (S fuel) decEqX (S k) l@(x :: xs) (y :: ys) (z :: zs) prf prfElem {len = S len'} with (filterX' decEqX x xs) proof eq_fxxs
  combiX_sound1 (S fuel) decEqX (S k) l@(x :: xs) (y :: ys) (z :: zs) prf prfElem {len = S len'} | (m_fxxs ** v_fxxs) with (combiX decEqX k v_fxxs) proof eq_ckvfxxs
    combiX_sound1 (S fuel) decEqX (S k) l@(x :: xs) (y :: ys) (z :: zs) prf prfElem {len = S len'} | (m_fxxs ** v_fxxs) | (m_ckvfxxs ** v_ckvfxxs) with (combiX decEqX (S k) xs) proof eq_cskxs
      combiX_sound1 (S fuel) decEqX (S k) l@(x :: xs) (y :: ys) (z :: zs) prf prfElem {len = S len'} | (m_fxxs ** v_fxxs) | (m_ckvfxxs ** v_ckvfxxs) | (m_cskxs ** v_cskxs) with (decEq x y)
        combiX_sound1 (S fuel) decEqX (S k) l@(y :: xs) (y :: ys) (z :: zs) prf prfElem {len = S len'} | (m_fxxs ** v_fxxs) | (m_ckvfxxs ** v_ckvfxxs) | (m_cskxs ** v_cskxs) | (Yes Refl) =
            ...
            case ei of
                (Left leftPrf) => 
                    ...
                    let (LTESucc cond1) = cond in
                    let t = prop_filterX_len decEqX m_fxxs v_fxxs {w=y} len' xs (sym eq_fxxs) in
                    let t' = prop_lte_2xp1 m_fxxs len' t in
                    let cond' = transitive t' cond1 in
                    let ih = combiX_sound1 fuel decEqX k {cond = cond'} v_fxxs ys v_ckvfxxs (sym eq_ckvfxxs) ttt in
                ...
                (Right rightPrf) => 
                    let (LTESucc cond1) = cond in
                    let cond1' = replace {p = \c => LTE (c) fuel} (sym $ plusSuccRightSucc len' (S len')) cond1 in
                    let cond' = prop_lte (plus len' (S len')) fuel cond1' in
                    let ih = combiX_sound1 fuel decEqX (S k) {cond = cond'} xs (y :: ys) _ (sym eq_cskxs) rightPrf in
                    SVSkip ih
        combiX_sound1 (S fuel) decEqX (S k) l@(x :: xs) (y :: ys) (z :: zs) prf prfElem {len = S len'} | (m_fxxs ** v_fxxs) | (m_ckvfxxs ** v_ckvfxxs) | (m_cskxs ** v_cskxs) | (No ne_xy) =
            ...
            let (LTESucc cond1) = cond in
            let cond1' = replace {p = \c => LTE (c) fuel} (sym $ plusSuccRightSucc len' (S len')) cond1 in
            let cond2 = prop_lte (plus len' (S len')) fuel cond1' in
            let ih = combiX_sound1 fuel decEqX (S k) {cond = cond2} xs (y :: ys) _ (sym eq_cskxs) t in
            SVSkip ih
...
combiX_sound2_sub2 0        _ _ _ {cond} _ _ _ _ _ =
    let cond' = replace {p = \c => LTE (c) 0} (sym $ plusSuccRightSucc _ len) cond in
    absurd cond'
combiX_sound2_sub2 (S fuel) decEqX [] y [] ([] :: []) Refl Here prfNE = prfNE
combiX_sound2_sub2 (S fuel) decEqX [] y [] ([] :: []) Refl (There later) prfNE = prfNE
combiX_sound2_sub2 (S fuel) decEqX (x :: xs) y [] [[]] Refl prfElem prfNE = \arg => absurd arg
combiX_sound2_sub2 (S fuel) decEqX [] _ _ [] Refl Here _ impossible
combiX_sound2_sub2 (S fuel) _ [] _ _ [] Refl (There later) _ impossible
combiX_sound2_sub2 (S fuel) _ (_ :: _) _ [] [] _ Here _ impossible
combiX_sound2_sub2 (S fuel) _ (_ :: _) _ [] [] _ (There _) _ impossible
combiX_sound2_sub2 (S fuel) _ (_ :: _) _ (_ :: _) [] _ Here _ impossible
combiX_sound2_sub2 (S fuel) _ (_ :: _) _ (_ :: _) [] _ (There later) _ impossible
combiX_sound2_sub2 (S fuel) _ _ _ [] (_ :: _ :: _) _ _ _ = absurd
combiX_sound2_sub2 (S fuel) decEqX {n=S k} [] _ (_ :: _) (_ :: _) prf _ _ = absurd (cong fst prf)
combiX_sound2_sub2 (S fuel) decEqX {n=S k} l@(x :: xs) v {len = S len'} (y :: ys) (z :: zs) prf prfElem prfNE with (filterX' decEqX x xs) proof eq_fxxs
  combiX_sound2_sub2 (S fuel) decEqX {n=S k} l@(x :: xs) v {len = S len'} (y :: ys) (z :: zs) prf prfElem prfNE | (m_fxxs ** v_fxxs) with (combiX decEqX k v_fxxs) proof eq_ckvfxxs
    combiX_sound2_sub2 (S fuel) decEqX {n=S k} l@(x :: xs) v {len = S len'} (y :: ys) (z :: zs) prf prfElem prfNE | (m_fxxs ** v_fxxs) | (m_ckvfxxs ** v_ckvfxxs) with (combiX decEqX (S k) xs) proof eq_cskxs
      combiX_sound2_sub2 (S fuel) decEqX {n=S k} l@(x :: xs) v {len = S len'} (y :: ys) (z :: zs) prf prfElem prfNE | (m_fxxs ** v_fxxs) | (m_ckvfxxs ** v_ckvfxxs) | (m_cskxs ** v_cskxs) with (decEq x y)
        combiX_sound2_sub2 (S fuel) decEqX {n=S k} l@(y :: xs) v {len = S len'} (y :: ys) (z :: zs) prf prfElem prfNE | (m_fxxs ** v_fxxs) | (m_ckvfxxs ** v_ckvfxxs) | (m_cskxs ** v_cskxs) | (Yes Refl) =
            let prfElem' = replace {p = \dp => Elem (y :: ys) (snd dp)} prf prfElem in
            let ei = prop_Elem_conc_either (prepend y v_ckvfxxs) v_cskxs prfElem' in
            case ei of
                (Left leftPrf) => 
                    ...
                    let tt = prop_filterX_noelem2X decEqX v xs v_fxxs (sym eq_fxxs) prfNE' in
                    let (LTESucc cond1) = cond in
                    let t = prop_filterX_len decEqX m_fxxs v_fxxs {w=y} len' xs (sym eq_fxxs) in
                    let t' = prop_lte_2xp1 m_fxxs len' t in
                    let cond' = transitive t' cond1 in
                    let ih = combiX_sound2_sub2 fuel decEqX {n=k} v_fxxs v {cond = cond'} ys v_ckvfxxs (sym eq_ckvfxxs) leftPrf' tt in
                ...
                (Right rightPrf) =>
                    ...
                    let cond1' = replace {p = \c => LTE (c) fuel} (sym $ plusSuccRightSucc len' (S len')) cond1 in
                    let cond' = prop_lte (plus len' (S len')) fuel cond1' in
                    combiX_sound2_sub2 fuel decEqX {n=S k} xs v {cond = cond'} (y :: ys) v_cskxs (sym eq_cskxs) rightPrf prfNE'
        combiX_sound2_sub2 (S fuel) decEqX l@(x :: xs) v {len = S len'} (y :: ys) (z :: zs) prf prfElem prfNE | (m_fxxs ** v_fxxs) | (m_ckvfxxs ** v_ckvfxxs) | (m_cskxs ** v_cskxs) | (No ne_xy) =
            let fact = prependElemNegative y x ys v_ckvfxxs ne_xy in
            let prfElem' = replace {p = \dp => Elem (y :: ys) (snd dp)} prf prfElem in
            let t = prop_Elem_conc_2nd (y :: ys) (prepend x v_ckvfxxs) v_cskxs prfElem' fact in
            ...
            let cond1' = replace {p = \c => LTE (c) fuel} (sym $ plusSuccRightSucc len' (S len')) cond1 in
            let cond2 = prop_lte (plus len' (S len')) fuel cond1' in
            combiX_sound2_sub2 fuel decEqX {n=S k} xs v {cond = cond2} (y :: ys) v_cskxs (sym eq_cskxs) t prfNE'
...
combiX_sound2 :
    ...
    (fuel : Nat) ->
    (dec : ((z, y : ty) -> Dec (rel z y))) ->
    (n : Nat) -> {len : Nat} -> (v : Vect n ty) -> (l : Vect len ty) -> -- (y : ty) ->
    {cond : LTE (plus len (S len)) fuel} ->
    {m : Nat} -> (l' : Vect m (Vect n ty)) ->
    (prf : (m ** l') = combiX dec n l) -> (prfElem : Elem v l') -> NoDupX ty rel v
--         dec n  l v   l' prf prfElem
combiX_sound2 0        _ _ _ {cond} _ _ _ _ =
    -- cond : LTE (plus (length _) (S len)) 0
    let cond' = replace {p = \c => LTE (c) 0} (sym $ plusSuccRightSucc _ len) cond in
    absurd cond'
combiX_sound2 (S fuel) _ 0 [] [] ([] :: []) Refl Here = NDXNil
combiX_sound2 (S fuel) _ 0 [] [] ([] :: []) Refl (There later) = NDXNil
combiX_sound2 (S fuel) _ 0 [] (y :: ys) ([] :: []) Refl Here = NDXNil
combiX_sound2 (S fuel) _ 0 [] (y :: ys) ([] :: []) Refl (There later) = NDXNil
combiX_sound2 (S fuel) _ (S k) l' [] (l' :: xs) prf Here = absurd (cong fst prf)
combiX_sound2 (S fuel) _ (S k) l' [] (y :: xs) prf (There later) = absurd (cong fst prf)
combiX_sound2 (S fuel) _ (S _) (_ :: _) (_ :: _) [] _ Here impossible
combiX_sound2 (S fuel) _ (S _) (_ :: _) (_ :: _) [] _ (There later) impossible
combiX_sound2 (S fuel) decEqX (S k) {cond} (y :: ys) l@(x :: xs) (z :: zs) prf prfElem {len = S len'} with (filterX' decEqX x xs) proof eq_fxxs
  combiX_sound2 (S fuel) decEqX (S k) {cond} (y :: ys) l@(x :: xs) (z :: zs) prf prfElem {len = S len'} | (m_fxxs ** v_fxxs) with (combiX decEqX k v_fxxs) proof eq_ckvfxxs
    combiX_sound2 (S fuel) decEqX (S k) {cond} (y :: ys) l@(x :: xs) (z :: zs) prf prfElem {len = S len'} | (m_fxxs ** v_fxxs) | (m_ckvfxxs ** v_ckvfxxs) with (combiX decEqX (S k) xs) proof eq_cskxs
      combiX_sound2 (S fuel) decEqX (S k) {cond} (y :: ys) l@(x :: xs) (z :: zs) prf prfElem {len = S len'} | (m_fxxs ** v_fxxs) | (m_ckvfxxs ** v_ckvfxxs) | (m_cskxs ** v_cskxs) with (decEq x y)
        combiX_sound2 (S fuel) decEqX (S k) {cond} (y :: ys) l@(y :: xs) (z :: zs) prf prfElem {len = S len'} | (m_fxxs ** v_fxxs) | (m_ckvfxxs ** v_ckvfxxs) | (m_cskxs ** v_cskxs) | (Yes Refl) =
            let prfElem' = replace {p = \dp => Elem (y :: ys) (snd dp)} prf prfElem in
            let ei = prop_Elem_conc_either (prepend y v_ckvfxxs) v_cskxs prfElem' in
            case ei of
                (Left leftPrf) => 
                    ...
                    let t' = prop_lte_2xp1 m_fxxs len' t in
                    let cond' = transitive t' cond1 in
                    let ih = combiX_sound2 fuel decEqX k {cond = cond'} ys v_fxxs v_ckvfxxs (sym eq_ckvfxxs) leftPrf' in
                    let tX = prop_filterX_noelem decEqX y xs v_fxxs (sym eq_fxxs) in
                    let u2 = combiX_sound2_sub2 fuel decEqX v_fxxs {cond = cond'} y ys v_ckvfxxs (sym eq_ckvfxxs) leftPrf' tX in
                    NDXCons y ys ih (\arg => u2 arg)
                (Right rightPrf) => 
                    let (LTESucc cond1) = cond in
                    let cond1' = replace {p = \c => LTE (c) fuel} (sym $ plusSuccRightSucc len' (S len')) cond1 in
                    let cond' = prop_lte (plus len' (S len')) fuel cond1' in
                    combiX_sound2 fuel decEqX (S k) {cond = cond'} (y :: ys) xs v_cskxs (sym eq_cskxs) rightPrf
        combiX_sound2 (S fuel) decEqX (S k) (y :: ys) l@(x :: xs) (z :: zs) prf prfElem {len = S len'} | (m_fxxs ** v_fxxs) | (m_ckvfxxs ** v_ckvfxxs) | (m_cskxs ** v_cskxs) | (No ne_xy) =
            let fact = prependElemNegative y x ys v_ckvfxxs ne_xy in
            let prfElem' = replace {p = \dp => Elem (y :: ys) (snd dp)} prf prfElem in
            let t = prop_Elem_conc_2nd (y :: ys) (prepend x v_ckvfxxs) v_cskxs prfElem' fact in
            let (LTESucc cond1) = cond in
            let cond1' = replace {p = \c => LTE (c) fuel} (sym $ plusSuccRightSucc len' (S len')) cond1 in
            let cond2 = prop_lte (plus len' (S len')) fuel cond1' in
            combiX_sound2 fuel decEqX (S k) {cond = cond2} (y :: ys) xs v_cskxs (sym eq_cskxs) t
